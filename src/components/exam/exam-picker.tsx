...
export function ExamPicker({
  ...
  const { sortBy, sortDirection, setSortBy, setSortDirection } = useExamSortPreference("exam-picker");
  const sorted = useMemo(() => sortExams(exams, sortBy, sortDirection), [exams, sortBy, sortDirection]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align={align} sideOffset={8} className="w-auto overflow-hidden p-0">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  current ? "bg-accent font-semibold" : "text-foreground/90 hover:bg-muted hover:text-foreground",
                )}
                onClick={() => pick(exam)}
              >
                <span className="truncate text-sm font-normal">{getExamPrefix(exam)}</span>
                <span className="text-sm font-semibold tabular-nums">{exam.exam_date}</span>
                {exam.has_solution ? (
                  <Badge variant="secondary" className="col-start-3 justify-self-start">
                    Facit
                  </Badge>
                ) : (
                  <span />
                ...
                <span
                  className={cn(
                    ...
                    hasPassRate(exam) ? passRateClass(exam.pass_rate) : "text-muted-foreground/50",
                  )}
                >
                  {hasPassRate(exam) ? `${Number(exam.pass_rate).toFixed(1)}%` : "–"}
                </span>
                {current ? (
