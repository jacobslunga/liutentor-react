import { FileTextIcon, GlobeIcon, ImageIcon, LoaderCircleIcon } from "lucide-react";
import { memo, useCallback, useEffect, useImperativeHandle, useRef, useState, type Ref, type RefObject } from "react";
import { useShallow } from "zustand/react/shallow";
import { Button } from "@/components/ui/button";
import { useChatMarkdownReady } from "@/hooks/use-chat-markdown";
import { renderCachedChatMarkdown, renderChatMarkdown, selectionToMarkdown } from "@/lib/chat-markdown";
import { formatFileSize } from "@/lib/format";
import { cn } from "@/lib/utils";
import { useChatStore, type ChatAttachment, type MessageSource } from "@/stores/chat";
 ...
 */
export function ChatMessages({ ref, scrollRef, className, onReplyToSelection }: ChatMessagesProps) {
  const ids = useChatStore(useShallow((s) => s.messages.map((m) => m.id)));
  const mdReady = useChatMarkdownReady();
  const rootRef = useRef<HTMLDivElement>(null);
  const [popover, setPopover] = useState<{ x: number; y: number; anchor: number } | null>(null);
  ...
  const scrollUserMessageToTop = useCallback(() => {
    ...
    const reserve = Math.max(parent.clientHeight - lastUser.offsetHeight - gap - 40, 0);
  ...
  useEffect(() => {
    if (!mdReady) return;
    if (useChatStore.getState().isLoading && useChatStore.getState().savedScrollPosition === null) {
  ...
  function onMouseUp() {
    setTimeout(() => {
      ...
      if (!selection?.toString().trim() || !selection.rangeCount) return setPopover(null);
      const range = selection.getRangeAt(0);
      const node = range.commonAncestorContainer;
      const el = node.nodeType === Node.TEXT_NODE ? node.parentElement : (node as Element);
      if (!el?.closest('[data-role="assistant"]')) return setPopover(null);
      const rect = range.getBoundingClientRect();
      setPopover({ x: rect.left + rect.width / 2, y: rect.top, anchor: scrollRef.current?.scrollTop ?? 0 });
    }, 0);
  }
  ...
  if (!mdReady) {
    return (
      <div role="status" className={cn("flex flex-1 items-center justify-center gap-2 px-4 py-20 text-sm text-muted-foreground", className)}>
  ...
  return (
    <>
      ...
      {popover && <SelectionPopover x={popover.x} y={popover.y} onReply={reply} />}
    </>
  );
}
...
const MessageRow = memo(function MessageRow({ id, isLast }: { id: string; isLast: boolean }) {
  ...
  if (message.role === "user") {
    return (
      <div data-role="user" className="flex min-w-0 scroll-mt-20 justify-end py-2">
  ...
  const showStatus = !!message.status?.message || (!message.content && isStreaming);
  ...
  return (
    <div data-role="assistant" className="w-full min-w-0 overflow-hidden pt-2 pb-8">
      {showStatus && (
        <div className="mb-2 flex h-6 items-center gap-2">
          <LoaderCircleIcon className="variable-spin size-4 text-muted-foreground" />
          <span className="shimmer-text text-sm">{message.status?.message || loadingPhrase}</span>
        </div>
      )}
      {html && (
        <div
      ...
      {!!message.sources?.length && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {message.sources.map((source) => (
            <Button key={source.url} asChild variant="outline" size="xs" className="max-w-56">
  ...
  return (
    ...
    >
      {attachment.mediaType === "application/pdf" ? (
        ...
        <img src={attachment.previewUrl} alt="" className="size-14 shrink-0 rounded-md object-cover" />
      ) : (
